...
# ADDED BY THOMAS
#########################
testanalyzer = cfg.Analyzer(
    MECoreAnalyzers.TestAnalyzer,
    'test',
    _conf = conf
)
...
leptonType = NTupleObjectType("leptonType", variables = [
    ...
    NTupleVariable("is_hadr", lambda x : x.is_hadr if hasattr(x, "is_hadr") else -1),
    NTupleVariable("jet_delR", lambda x : x.jet_delR if hasattr(x, "jet_delR") else -1),
    NTupleVariable("subjet_delR", lambda x : x.subjet_delR if hasattr(x,"subjet_delR") else -1),
    #NTupleVariable("mcPt", lambda x : x.mcPt),
    #NTupleVariable("mcEta", lambda x : x.mcEta),
    #NTupleVariable("mcPhi", lambda x : x.mcPhi),
...
delRType = NTupleObjectType("delRType", variables = [
    NTupleVariable("delR", lambda x : x.delR),
])
...
treeProducer = cfg.Analyzer(
    ...
    globalVariables = [
        ...
        # Added by Thomas <-- WORKS
#        NTupleVariable(
#            "th_n_good_jets", lambda ev: ev.th_n_good_jets,
#            help="test by Thomas: n_good_jets"
#        ),


#        NTupleVariable(
#            "GenQ_Jets_delR", lambda ev: ev.GenQ_Jets_delR if hasattr(ev, "GenQ_Jets_delR") else -1,
#            help="Delta R values of the Gen(BWZ)Quarks matched to good_jets"
#        ),

        NTupleVariable(
            "GenQ_jet_sumdelR", lambda ev: ev.GenQ_jet_sumdelR if hasattr(ev, "GenQ_jet_sumdelR") else -1,
            help="Sum of Delta R values of the Gen(BWZ)Quarks matched to good_jets"
        ),

        NTupleVariable(
            "GenQ_subjet_sumdelR", lambda ev: ev.GenQ_subjet_sumdelR if hasattr(ev, "GenQ_subjet_sumdelR") else -1,
    ...
    #standard dumping of objects
        ...
        #"GenQ_Jets" : NTupleCollection("GenQ_Jets", delRType, 1, help=""),
    }
)
