...
def Make_config():
    ...
    config['input_root_file_name'] = '/scratch/tklijnsm/V10_limited_jets_TTJets.root'

    config['input_tree_name'] = 'tree'

    config['outputdir'] = 'V10_limited_TF_jets_lDG_newinit'

        
    config['SBF_fitdata_pickle_filename'] = '{0}/SBF_fitdata_pickle.dat'.format(
        config['outputdir'])

    config['SBF_hists_pickle_filename'] = '{0}/SBF_hists_pickle.dat'.format(
        config['outputdir'])    
    ...
    setattr( DG_func, 'fitrange', (30.0, 500.0) )
    ...
    config['bin_functions']['l']= [DG_func for i in range(len(config['eta_axis'])-1)]
