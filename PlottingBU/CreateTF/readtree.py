...
def Single_Bin_Fits():
        ...
        for i_eta in range( dic['n_eta_bins'] ):
            ...
            for i_E in range( dic['n_E_bins'] ):
                ...
                if hasattr( fitfunc, 'Is_DG' ):
                    f1 = Fit_DG( dic['hist_mat'][i_eta][i_E], f1, config )
                ...
                fit_dict['eta_bounds'] = [ eta_axis[i_eta], eta_axis[i_eta+1] ]
                ...
                fit_dict['single_bin_func'] = fitfunc
    ...
    print 'Writing single bin fit data to {0}.dat'.format(
        config['SBF_fitdata_pickle_filename'] )
    picklef = open( config['SBF_fitdata_pickle_filename'], 'wb' )
